        ...
        for(let key in this.models){
            ...
            for(let attr_name in attrs){
                ...
                if(attr.include !== false){
                    ...
                    ntypes += util.createRow(attr_name, g_type)
    ...
    loadGetSingle(model){
        ...
        if (!this.Query[query_name]) {
            ...
            this.Query[query_name] = async (parent, args, {}, info) => {
                ...
                let data;
                try {
                    data = await actualModel.findOne({where: args, include: includes});
                } catch (err) {
                    console.log('err', err);
                    return false
                }
    ...
    loadGetterQuery(model){
        ...
        if (!this.Query[all_query_name]) {
            ...
            this.Query[all_query_name] = async (parent, args, {}, info) => {
                ...
                let includes = util.getIncludes(requested, model, this.sequelize.models);
