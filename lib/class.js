...
class GraphApi {
    ...
    loadAssociations(model, ntypes){
        ...
        for( let key in assocs){
            ...
            let referenceName = assoc.schemaType;
            let relation = assoc.relation;
    ...
    composeTypesAndResolvers(){
        ...
        for(let key in this.models){
            ...
            for(let attr_name in attrs){
                ...
                let schemaType = attr.schemaType
                if(attr.relation == 'many'){
                    schemaType = `[${attr.schemaType}]`;
                }
                ntypes += util.createRow(attr.name, attr.schemaType);//is just included in query

                inputTypes+= util.createRow(attr.name, attr.schemaType);
            }
    ...
    loadGetSingle(model){
        if(!model.queryGetSingle) return;
