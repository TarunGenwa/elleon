        ...
        if(this.resolver_path){
          fs.readdirSync(this.resolver_path)
            ...
            for (let res in resolver){
              ...
              let returnType;
              if(_.isString(val.returnType)){
                  returnType = val.returnType;
              }else if(_.isArray(val.returnType)){
                  let inside = val.returnType[0];
                  let insideString;
                  if(_.isString(inside)){
                      insideString = inside;
                  }
                  returnType = `[${insideString}]`;
              }else if(_.isFunction(val.returnType)){
                  let inside = val.returnType();
                  let insideString;
                  if(_.isString(inside)){
                      insideString = inside;
              ...
              this[schemaTypeObjectName] += `\t${res}: ${returnType}\n`;
            }
