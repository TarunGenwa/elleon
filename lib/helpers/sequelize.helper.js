...
const loadProperties = module.exports.loadAttributes = (model) => {
    ...
    let modelGqlOptions = (model.graphqlOptions) ? model.graphqlOptions : defaultGqlOptions;
    let gqlOptions = {};
    Object.assign(gqlOptions, defaultGqlOptions, modelGqlOptions);


    let removeAttr = gqlOptions.removeAttributes;
    for(let name in props){
        let prop = props[name];

        if(removeAttr.includes(name)) continue;
        ...
        if(prop.graphql) {
            if(typeof prop.graphql.getQueryAttribute !== 'undefined') formattedProp.parameterOptions.getQueryAttribute = prop.graphql.getQueryAttribute;
            if(typeof prop.graphql.getQueryInput !== 'undefined') formattedProp.parameterOptions.getQueryInput = prop.graphql.getQueryInput;
        }

        formatted[name] = new Attribute(formattedProp);
    }

    let additional = gqlOptions.addAttributes;
    ...
    if(!_.isEmpty(additional)){
        for(let name in additional){
            ...
            let g_type = util.getType(attr.schemaType);

            let relation = (attr.relation) ? attr.relation : 'one';
            ...
            formatted[name] = new Attribute(formattedAttr);
        }
    }
...
const loadAssociations = module.exports.loadAssociations = (model) => {
    ...
    for( let assocName in assocs){
        ...
        let formattedAssoc = {
            name:assocName,
            schemaType: util.capitalizeFirstLetter(referenceName),
            relation:relation,
            actual:assoc,
            type:'association'
        };
