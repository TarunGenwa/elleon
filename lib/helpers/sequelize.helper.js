...
const AbstractAllModels = module.exports.AbstractAllModels = (models) => {
    ...
    for(let key in models){
        ...
        if(fmodels.models[model.name]) throw `duplicate model name: ${model.name}`;
...
const addGetAll = module.exports.addGetAll = async (instance, model) => {
    ...
    if (!instance.Query[model.queryAllName]) {

        instance.queries += instance.makeRowStr(model.queryAllName, model, {association: 'many', limit:true});

        instance.Query[model.queryAllName] = async (parent, args, {}, info) => {
